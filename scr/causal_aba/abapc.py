...
# ─── Below disabled: only `get_cg_and_facts` is used in the single-seed flow. ───
# Several of these imports reference modules that do not exist in this repo
# (e.g. `src.utils.fact_utils`, `src.causal_aba.factory`, `logger`), so leaving
# them active breaks module load.
#
...
# ─── Everything below disabled: not called by the single-seed flow. ─────────
# Many of these functions depend on imports that have been commented out above
# and/or on modules that don't exist in this repo. Uncomment and fix imports
# if you need them.
#
...
# @dataclass
# class BestModel:
#     best_model: frozenset
#     best_B_est: np.ndarray
#     best_I: float
#     elapsed: float
#
#
