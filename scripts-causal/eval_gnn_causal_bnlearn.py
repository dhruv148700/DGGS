"""
eval_gnn_causal_bnlearn.py
──────────────────────────
Run the GNN causal extension generator on the no-leakage bnlearn causal dataset
(no_leakage_data_bnlearn_causal/aba/) and print per-sample + per-dataset summary
...
Usage:
  python scripts-causal/eval_gnn_causal_bnlearn.py --model-type gcn
  python scripts-causal/eval_gnn_causal_bnlearn.py --model-type gat --model-path path/to/model.pt
  python scripts-causal/eval_gnn_causal_bnlearn.py --model-type gcn --seeds-per-dataset 5
  python scripts-causal/eval_gnn_causal_bnlearn.py --model-type gcn --datasets cancer earthquake
  python scripts-causal/eval_gnn_causal_bnlearn.py --model-type gcn --no-reject-edge-on-indep
"""
...
from scr.gnn_causal_extension_generator import build_gnn_extension
...
def evaluate_dataset(
    ...
    for seed_idx, seed in enumerate(seeds):
        ...
        scores_path = os.path.join(ABA_DIR, f"causal_{stem}_full.scores.json") if use_scores else None
        ...
        if scores_path and not os.path.exists(scores_path):
            scores_path = None

        print(f"  [{seed_idx+1}/{len(seeds)}] seed={seed:4d} ...", end=" ", flush=True)
        t0 = time.time()
        try:
            extension, _ = build_gnn_extension(
                aba_path,
...
def main():
    ...
    p.add_argument("--model-type", type=str, default=None, choices=["gcn", "gat"],
                   help="GNN model type (inferred from results.json if --results-dir given)")
    p.add_argument("--model-path", type=str, default=None,
                   help="Path to trained model .pt file")
    p.add_argument("--results-dir", type=str, default=None,
                   help="Path to training results dir containing results.json and best_model.pt. "
                        "Reads model_type and all hyperparams automatically.")
    ...
    if args.results_dir:
        ...
        hp_override = {
            ...
            "hidden_features": hp["hidden_dim"],
            "embedding_dim":  hp["embedding_dim"],
            "num_layers":     hp["num_layers"],
            "dropout":        hp["dropout"],
        }
        use_scores = hp["in_features"] == 3
        if args.model_path is None:
            args.model_path = os.path.join(args.results_dir, "best_model.pt")
    else:
        if args.model_type is None:
            p.error("--model-type is required when --results-dir is not provided")
        model_type = args.model_type

    model_path = args.model_path
    if model_path is None:
        model_path = os.path.join(REPO_ROOT, f"results_final_{model_type}", "trained_model.pt")
    ...
    print(f"Use scores (3-feat): {use_scores}")
    ...
    inference_engine = ABAInferenceEngine(model_type, model_path, **hp_override)
